...
`.replaceAll(/\r?\n/g, '\r\n')
      ...
      const request = new Request('https://catbox.moe/user/api.php', {
        method: 'POST',
        headers: {
  ...
  async download(
    ...
    try {
      ...
      if (!fileResponse.ok) {
        console.error(`Failed to download from Catbox: ${fileResponse.statusText}`)
        return null
      }

      return fileResponse
    } catch (error) {
      console.error('Catbox download failed:', error)
      return null
    }
  }
}
